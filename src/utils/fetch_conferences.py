...
def fetch_conference_abstracts(term, limit=50):
    ...
    print(f"Querying Europe PMC for conference abstracts matching: '{term}' (limit: {limit})...")
    ...
    while len(results) < limit:
        ...
        try:
            ...
            print(f"Error querying Europe PMC for conferences at cursor {cursor_mark}: {e}", file=sys.stderr)
            break
            
    return results[:limit]

def main():
    parser = argparse.ArgumentParser(description="Query Europe PMC for oncology/hematology conference abstracts")
    parser.add_argument("--term", required=True, help="Search term (e.g., drug name or target)")
    parser.add_argument("--output", default="tmp/conferences_out.json", help="Path to save raw JSON")
    ...
    if not results:
        print(f"Error: No conference abstracts found for '{args.term}' inside major meeting libraries.", file=sys.stderr)
        sys.exit(1)
