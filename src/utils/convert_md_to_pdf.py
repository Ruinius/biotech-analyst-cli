...
def markdown_table_to_html(
    markdown_table_text: str, table_class: str = "", col_widths: list = None
) -> str:
    import math
    ...
    if not col_widths:
        ...
        min_widths = [
            6.0 if "phase" in headers[i].lower() else 8.0 for i in range(num_cols)
        ]
    ...
    html = []
    html.append(
        f'<table class="{table_class}" style="width: 100%; border-collapse: collapse; margin-top: 1em; margin-bottom: 1em; page-break-inside: avoid; table-layout: fixed;">'
    )
    ...
    for idx, h in enumerate(headers):
        ...
        width_style = (
            f" width: {col_widths[idx]};"
            if col_widths and idx < len(col_widths)
            else ""
        )
        html.append(
            f'      <th style="padding: 8px 10px; font-weight: 600; text-align: left; border-bottom: 2px solid #cbd5e1; color: #0f172a; font-size: 9.5pt;{width_style}">{formatted_h}</th>'
        )
    html.append("    </tr>")
    html.append("  </thead>")
    ...
    for r_idx, row in enumerate(rows):
        ...
        for idx, cell in enumerate(row):
            ...
            formatted_cell = re.sub(
                r"\*\*(.*?)\*\*", r"<strong>\1</strong>", formatted_cell
            )
            ...
            html.append(
                f'      <td style="padding: 8px 10px; border-bottom: 1px solid #e2e8f0; vertical-align: top; color: #1f2937; font-size: 9pt; line-height: 1.45;{width_style}">{formatted_cell}</td>'
            )
        html.append("    </tr>")
    html.append("  </tbody>")
    html.append("</table>")
...
def clean_markdown_math(text: str) -> str:
    ...
    text = text.replace(
        "K_D = 0.398 \\text{ \\text{pM}}", "<i>K</i><sub>d</sub> = 0.398 pM"
    )
    text = text.replace(
        "K_D = 0.398 \\text{\\text{pM}}", "<i>K</i><sub>d</sub> = 0.398 pM"
    )
...
def add_header_footer_to_pdf(
    pdf_path: str, header_left: str = None, footer_left: str = None
):
    print("Post-processing PDF to add headers, footers and page numbers...")
    if not header_left:
        header_left = (
            "Biotech BD Pipeline Summary: Clinical & Preclinical Asset Landscape"
        )
    if not footer_left:
    ...
    for page in doc:
        ...
        if page.number > 0:
            ...
            page.insert_text(
                (margin_left, header_y), header_left, fontsize=8, color=(0.4, 0.4, 0.4)
            )

            # Right-aligned header text using insert_textbox
            page.insert_textbox(
                pymupdf.Rect(
                    margin_right - 250, header_y - 8, margin_right, header_y + 12
                ),
                header_right,
                fontsize=8,
                color=(0.4, 0.4, 0.4),
        ...
        page.insert_text(
            (margin_left, footer_y), footer_left, fontsize=8, color=(0.4, 0.4, 0.4)
        )
...
"""
    pdf.add_section(
        Section(md_content, paper_size="A4-L", borders=(36, 36, -36, -36)), user_css=css
    )
    pdf.save(pdf_path)

    # Post-process to add headers, footers and page numbers
    add_header_footer_to_pdf(
        pdf_path, header_left=header_title, footer_left=report_date
    )
...
"""
    ...
    print(
        "Splitting markdown into: Section 1 (Portrait), Section 2 (Landscape), Section 3 (Portrait)"
    )
...
"""
    ...
    pdf.add_section(
        Section(part_1, paper_size="A4", borders=(54, 54, -54, -54)), user_css=css
    )

    # Section 2: Landscape with narrower 0.5in margins (36pt) to give table maximum width
    pdf.add_section(
        Section(part_2, paper_size="A4-L", borders=(36, 36, -36, -36)), user_css=css
    )

    # Section 3a: Osemitamab Deep-Dive (Portrait)
    pdf.add_section(
        Section(part_3a, paper_size="A4", borders=(54, 54, -54, -54)), user_css=css
    )

    # Section 3b: TRK-950 Deep-Dive (Portrait)
    pdf.add_section(
        Section(part_3b, paper_size="A4", borders=(54, 54, -54, -54)), user_css=css
    )

    # Section 3c: STF32 Deep-Dive (Portrait)
    pdf.add_section(
        Section(part_3c, paper_size="A4", borders=(54, 54, -54, -54)), user_css=css
    )

    # Section 3d: Remainder of Deep Dives and Reference Matrix (Portrait)
    pdf.add_section(
        Section(part_3d, paper_size="A4", borders=(54, 54, -54, -54)), user_css=css
    )
