...
def fetch_chinese_trials(term, limit=50):
    ...
    print(
        f"Querying Europe PMC/WHO ICTRP for: '{term}' / '{translated}' (limit: {limit})..."
    )
    ...
    while len(results) < limit:
        ...
        try:
            ...
            print(
                f"Error querying Chinese registries syndicates at cursor {cursor_mark}: {e}",
                file=sys.stderr,
            )
            break
...
def main():
    parser = argparse.ArgumentParser(
        description="Query Chinese Clinical Trial Registries via WHO syndicated systems"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output",
        default="tmp/chinese_registries_out.json",
    ...
    if not results:
        print(
            f"Error: No clinical trials found for '{args.term}' inside WHO syndicated Chinese registries.",
            file=sys.stderr,
        )
        sys.exit(1)
