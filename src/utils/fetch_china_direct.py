...
def fetch_cde_direct(term, proxy_url=None):
    ...
    try:
        ...
        raise RuntimeError("Playwright is required to perform direct ChinaDrugTrials queries.") from e
    ...
    try:
        with sync_playwright() as p:
            ...
            if not response or response.status not in [200, 202]:
                ...
                raise RuntimeError(f"Failed to load ChinaDrugTrials portal. WAF blocked session with status: {status_code}")
            ...
            search_input = page.locator("input[name='keywords'], input.indexSearchInput").first
            ...
            for row in rows:
                try:
                    ...
                    if len(cols) >= 6:
                        ...
                        is_header = reg_num in ["登记号", "No.", "Acceptance Number", ""]
                        if reg_num and not is_header:
                            records.append({
                                ...
                                "status": status,
                                "breakthrough_therapy": "Yes" if "突破性" in status or "突破性" in ''.join(cols) else "No"
                            })
                except Exception:
                    pass
            ...
            if not records:
                raise ValueError(f"No authentic ChinaDrugTrials records found matching term: '{term}'.")
        ...
        print(f"Error executing live Playwright ChinaDrugTrials query: {e}", file=sys.stderr)
        raise RuntimeError(f"Playwright Direct ChinaDrugTrials search failed: {e}") from e

def main():
    parser = argparse.ArgumentParser(description="Query ChinaDrugTrials Official Portal directly with WAF bypass capabilities")
    parser.add_argument("--term", required=True, help="Search term (e.g., drug name or target)")
    parser.add_argument("--output", default="tmp/china_direct_out.json", help="Path to save raw JSON")
    parser.add_argument("--proxy", help="Optional proxy URL (e.g. http://127.0.0.1:8888)")
    ...
    try:
        ...
        print(f"Successfully saved ChinaDrugTrials direct search results to: {args.output}")
    except Exception as e:
