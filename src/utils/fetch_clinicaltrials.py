...
def fetch_studies(term, limit=50):
    ...
    while len(results) < limit:
        ...
        try:
            ...
            print(f"Error querying ClinicalTrials.gov API for '{term}' at page token {page_token}: {e}", file=sys.stderr)
            break
...
def main():
    parser = argparse.ArgumentParser(description="Query ClinicalTrials.gov API v2 (Windows & Unix Compatible)")
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument("--terms", nargs="+", help="One or more search terms (e.g., drug name or target)")
    group.add_argument("--nct-ids", nargs="+", help="One or more ClinicalTrials.gov NCT identifiers")
    parser.add_argument("--output", default="tmp/trials_out.json", help="Path to write the raw JSON results")
    parser.add_argument("--limit", type=int, default=50, help="Maximum number of studies to fetch per term")
