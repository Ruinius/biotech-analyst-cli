...
def fetch_patent_data(term, limit=50):
    ...
    print(f"Querying Europe PMC / Lens Syndicate for Patents matching: '{term}' (limit: {limit})...")
    ...
    while len(results) < limit:
        ...
        try:
            ...
            print(f"Error querying Europe PMC / Lens Syndicate at cursor {cursor_mark}: {e}", file=sys.stderr)
            break
            
    return results[:limit]

def main():
    parser = argparse.ArgumentParser(description="Query Lens/Dimensions Syndicate for Global IP & Patents")
    parser.add_argument("--term", required=True, help="Search term (e.g., drug name or target)")
    parser.add_argument("--output", default="tmp/ip_lens_out.json", help="Path to save raw JSON")
    ...
    if not results:
        print(f"Error: No patent records found for '{args.term}' inside Lens/Dimensions syndicate registries.", file=sys.stderr)
        sys.exit(1)
