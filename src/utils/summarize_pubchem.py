...
def main():
    ...
    for row in rows:
        ...
        outcome = cell[outcome_idx] if outcome_idx is not None and outcome_idx < len(cell) else "N/A"
        aid = cell[aid_idx] if aid_idx is not None and aid_idx < len(cell) else "N/A"
        target = cell[target_gene_idx] if target_gene_idx is not None and target_gene_idx < len(cell) else "N/A"
        val = cell[value_idx] if value_idx is not None and value_idx < len(cell) else "N/A"
        act_type = cell[type_idx] if type_idx is not None and type_idx < len(cell) else "N/A"
        assay_name = cell[name_idx] if name_idx is not None and name_idx < len(cell) else "N/A"
    ...
    try:
        with open(args.output, "w", encoding="utf-8") as out:
            out.write("="*80 + "\n")
            out.write(f"PUBCHEM COMPOUND & BIOASSAY SUMMARY REPORT\n")
