...
def fetch_drug_data(drug_name):
    # Query Drug Labels
    label_query = f'openfda.brand_name:"{drug_name}" OR openfda.generic_name:"{drug_name}"'
    ...
    results = {
        "drug_name": drug_name,
        "label": label_data.get("results", [{}])[0] if label_data and "results" in label_data else {},
        "events": event_data.get("results", []) if event_data and "results" in event_data else []
    }
    return results

def main():
    parser = argparse.ArgumentParser(description="Query openFDA API for Drug Labels & Events")
    parser.add_argument("--drug", required=True, help="Drug brand or generic name (e.g., 'Aspirin')")
    parser.add_argument("--output", default="tmp/openfda_out.json", help="Path to save raw JSON")
