...
TEST_SUITE = [
    {
        ...
        "fetch_cmd": [sys.executable, "utils/fetch_clinicaltrials.py", "--terms", "Pembrolizumab", "--output", "tmp/test_clinicaltrials_out.json", "--limit", "120"],
        "sum_cmd": [sys.executable, "utils/summarize_clinicaltrials.py", "--input", "tmp/test_clinicaltrials_out.json", "--output", "tmp/test_clinicaltrials_sum.txt"],
    ...
    },
    {
        ...
        "fetch_cmd": [sys.executable, "utils/fetch_pubchem.py", "--compound", "Aspirin", "--output", "tmp/test_pubchem_out.json"],
        "sum_cmd": [sys.executable, "utils/summarize_pubchem.py", "--input", "tmp/test_pubchem_out.json", "--output", "tmp/test_pubchem_sum.txt"],
    ...
    },
    {
        ...
        "fetch_cmd": [sys.executable, "utils/fetch_openfda.py", "--drug", "Aspirin", "--output", "tmp/test_openfda_out.json"],
        "sum_cmd": [sys.executable, "utils/summarize_openfda.py", "--input", "tmp/test_openfda_out.json", "--output", "tmp/test_openfda_sum.txt"],
        ...
        "json_assertions": [
            ...
            lambda data: data["label"].get("openfda", {}).get("generic_name", [""])[0].upper() == "ASPIRIN"
        ],
        "txt_assertions": [
    ...
    },
    {
        ...
        "fetch_cmd": [sys.executable, "utils/fetch_anzctr_ctis.py", "--term", "Pembrolizumab", "--output", "tmp/test_anzctr_ctis_out.json", "--limit", "120"],
        "sum_cmd": [sys.executable, "utils/summarize_anzctr_ctis.py", "--input", "tmp/test_anzctr_ctis_out.json", "--output", "tmp/test_anzctr_ctis_sum.txt"],
    ...
    },
    {
        ...
        "fetch_cmd": [sys.executable, "utils/fetch_conferences.py", "--term", "Pembrolizumab", "--output", "tmp/test_conferences_out.json", "--limit", "120"],
        "sum_cmd": [sys.executable, "utils/summarize_conferences.py", "--input", "tmp/test_conferences_out.json", "--output", "tmp/test_conferences_sum.txt"],
        "output_json": "tmp/test_conferences_out.json",
        "output_txt": "tmp/test_conferences_sum.txt",
        "json_assertions": [
            lambda data: isinstance(data.get("results"), list),
            lambda data: len(data["results"]) == 120,
            lambda data: "abstractText" in data["results"][0] or "title" in data["results"][0]
        ],
        "txt_assertions": [
    ...
    },
    {
        ...
        "fetch_cmd": [sys.executable, "utils/fetch_chinese_registries.py", "--term", "Pembrolizumab", "--output", "tmp/test_chinese_registries_out.json", "--limit", "120"],
        "sum_cmd": [sys.executable, "utils/summarize_chinese_registries.py", "--input", "tmp/test_chinese_registries_out.json", "--output", "tmp/test_chinese_registries_sum.txt"],
    ...
    },
    {
        ...
        "fetch_cmd": [sys.executable, "utils/fetch_china_direct.py", "--term", "Aspirin", "--output", "tmp/test_china_direct_out.json"],
        "sum_cmd": [sys.executable, "utils/summarize_china_direct.py", "--input", "tmp/test_china_direct_out.json", "--output", "tmp/test_china_direct_sum.txt"],
    ...
    },
    {
        ...
        "fetch_cmd": [sys.executable, "utils/fetch_ip_lens.py", "--term", "Pembrolizumab", "--output", "tmp/test_ip_lens_out.json"],
        "sum_cmd": [sys.executable, "utils/summarize_ip_lens.py", "--input", "tmp/test_ip_lens_out.json", "--output", "tmp/test_ip_lens_sum.txt"],
...
def test_utility(spec):
    ...
    if not os.path.exists(spec['output_json']):
        print(f"[ERROR] FETCH ERROR: Output file {spec['output_json']} was not created!", file=sys.stderr)
        return False
        ...
        with open(spec['output_json'], "r", encoding="utf-8") as f:
            ...
            for idx, assertion in enumerate(spec["json_assertions"], 1):
                if not assertion(data):
                    print(f"[FAIL] JSON Content Assertion #{idx} failed for {spec['output_json']}!", file=sys.stderr)
                    return False
    except Exception as e:
        print(f"[ERROR] FETCH ERROR: Failed to parse generated JSON or assertion crashed: {e}", file=sys.stderr)
        return False
    ...
    if not os.path.exists(spec['output_txt']):
        print(f"[ERROR] SUMMARIZER ERROR: Output file {spec['output_txt']} was not created!", file=sys.stderr)
        return False
        
    try:
        with open(spec['output_txt'], "r", encoding="utf-8") as f:
            content = f.read()
            if not content:
                print(f"[ERROR] SUMMARIZER ERROR: Report {spec['output_txt']} is empty!", file=sys.stderr)
                return False
                
            # Content assertions for text report
            for assertion in spec["txt_assertions"]:
                if assertion not in content:
                    print(f"[FAIL] Text Assertion Failure: Keyword '{assertion}' not found in {spec['output_txt']}!", file=sys.stderr)
                    return False
    except Exception as e:
        print(f"[ERROR] SUMMARIZER ERROR: Read or assertion failure: {e}", file=sys.stderr)
        return False
...
def main():
    ...
    for spec in TEST_SUITE:
        try:
            ...
            print(f"[ERROR] Unexpected error testing {spec['name']}: {e}", file=sys.stderr)
            failed_tests += 1
