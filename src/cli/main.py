...
from src.core.config import load_config, save_config, config_exists, Settings, mask_key
...
def main_config():
    ...
    base_folder = typer.prompt("Enter base folder for research output", default=default_base)
    
    # Ask if they want to configure API keys
    configure_keys = typer.confirm("Would you like to configure API keys for LLM report drafting?", default=True)
...
def main_folder():
    ...
    formatting.speak("Here is your active research portfolio directories:", include_interjection=False)
    ...
    if choice in label_map:
        ...
        if sys.platform.startswith("win"):
            open_exp = typer.confirm("Would you like to open this folder in Windows Explorer?", default=True)
            if open_exp:
                try:
...
def run_pipeline_step(cmd_args, step_name):
    ...
    res = subprocess.run([sys.executable] + cmd_args, env=my_env, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    if res.returncode != 0:
        formatting.print_warning(f"Step '{step_name}' completed with warning or error (Code {res.returncode}).")
...
def main_bdscan(
    ...
    if action == "new":
        target_name = typer.prompt("Enter pathway/target biological name (e.g. CLDN18.2)")
        target_name = target_name.strip()
        if not target_name:
            formatting.print_error("Target name cannot be empty.")
            raise typer.Exit(1)
            
        en_terms = typer.prompt("Enter English search synonyms (comma-separated)", default=f"{target_name}, {target_name.replace('.', '')}")
        zh_terms = typer.prompt("Enter Mandarin search synonyms (comma-separated)", default=target_name)
        modality = typer.prompt("Enter modality filter (e.g. ADC, Bispecific, Small Molecule or empty)", default="")
        
        # Format target folder name
        today = datetime.date.today().strftime("%Y%m%d")
        folder_safe_name = "".join(c if c.isalnum() or c in ("-", "_") else "_" for c in target_name)
        ...
        formatting.speak("Ribosomes active! Commencing automated database fetching routines...")
        
        # 1. ClinicalTrials.gov
        for term in en_list:
            out_file = f"tmp/{folder_safe_name}_ct_{term.replace(' ', '_')}.json"
            run_pipeline_step(
                ["src/utils/fetch_clinicaltrials.py", "--terms", term, "--output", out_file, "--limit", "50"],
                f"ClinicalTrials.gov for '{term}'"
            )
            if os.path.exists(out_file):
                run_pipeline_step(
                    ["src/utils/summarize_clinicaltrials.py", "--input", out_file, "--output", out_file.replace(".json", "_sum.txt")],
        ...
        for term in en_list:
            ...
            run_pipeline_step(
                ["src/utils/fetch_anzctr_ctis.py", "--term", term, "--output", out_file, "--limit", "50"],
                f"ANZCTR/CTIS for '{term}'"
            )
            if os.path.exists(out_file):
                run_pipeline_step(
                    ["src/utils/summarize_anzctr_ctis.py", "--input", out_file, "--output", out_file.replace(".json", "_sum.txt")],
        ...
        for term in en_list:
            ...
            run_pipeline_step(
                ["src/utils/fetch_conferences.py", "--term", term, "--output", out_file, "--limit", "50"],
                f"Conferences for '{term}'"
            )
            if os.path.exists(out_file):
                run_pipeline_step(
                    ["src/utils/summarize_conferences.py", "--input", out_file, "--output", out_file.replace(".json", "_sum.txt")],
        ...
        for term in zh_list:
            ...
            run_pipeline_step(
                ["src/utils/fetch_chinese_registries.py", "--term", term, "--output", out_file, "--limit", "50"],
                f"Chinese WHO Registries for '{term}'"
            )
            if os.path.exists(out_file):
                run_pipeline_step(
                    ["src/utils/summarize_chinese_registries.py", "--input", out_file, "--output", out_file.replace(".json", "_sum.txt")],
        ...
        for term in zh_list:
            ...
            if os.path.exists(out_file):
                run_pipeline_step(
                    ["src/utils/summarize_china_direct.py", "--input", out_file, "--output", out_file.replace(".json", "_sum.txt")],
        ...
        for term in en_list:
            ...
            run_pipeline_step(
                ["src/utils/fetch_ip_lens.py", "--term", term, "--output", out_file, "--limit", "50"],
                f"Lens.org Patents for '{term}'"
            )
            if os.path.exists(out_file):
                run_pipeline_step(
                    ["src/utils/summarize_ip_lens.py", "--input", out_file, "--output", out_file.replace(".json", "_sum.txt")],
        ...
        for term in en_list[:2]:
            ...
            if os.path.exists(out_file):
                run_pipeline_step(
                    ["src/utils/summarize_pubchem.py", "--input", out_file, "--output", out_file.replace(".json", "_sum.txt")],
        ...
        import glob
        merged_trials = {}
        for f_path in glob.glob(f"tmp/{folder_safe_name}_ct_*.json") + glob.glob(f"tmp/{folder_safe_name}_anzctr_*.json"):
            try:
        ...
        formatting.speak("Fascinating transcripts! Copying summary logs to the research folder...")
        research_dir = target_dir / "research"
        for sum_file in glob.glob(f"tmp/{folder_safe_name}_*_sum.txt"):
            try:
                dest = research_dir / f"{Path(sum_file).name.replace('_sum.txt', '.md')}"
                with open(sum_file, "r", encoding="utf-8") as s:
                    text = s.read()
    ...
    run_pipeline_step(
        [
            ...
            "--clinicaltrials", merged_ct_file,
            "--china-direct", merged_china_file,
            "--output", table_out_file
        ],
    ...
    if os.path.exists(table_out_file):
        with open(table_out_file, "r", encoding="utf-8") as tf:
            table_md = tf.read()

    # Create the report document draft
    report_file = target_dir / "final_output" / f"meta_analysis_{folder_safe_name}_{datetime.date.today().strftime('%Y%m%d')}.md"
    ...
    run_pipeline_step(
        [
            "src/utils/convert_md_to_pdf.py",
            report_file,
            pdf_out
        ],
    ...
    if sys.platform.startswith("win") and pdf_out.exists():
        open_pdf = typer.confirm("Would you like to open the compiled PDF now?", default=True)
        if open_pdf:
            try:
...
def main_deepdive(
    ...
    if action == "new":
        ...
        trial_id = typer.prompt("Enter primary Clinical Trial ID (NCT or CTR ID)", default="NCT04818671")
        
        # Format target folder name
        today = datetime.date.today().strftime("%Y%m%d")
        folder_safe_name = "".join(c if c.isalnum() or c in ("-", "_") else "_" for c in asset_name)
        ...
        # --- Run fetches and summaries ---
        formatting.speak("Transcribing trials! Fetching registry records for diligence audit...")
        
        # 1. ClinicalTrials.gov by NCT or term
        ct_file = f"tmp/{folder_safe_name}_ct.json"
        if trial_id.strip().upper().startswith("NCT"):
            run_pipeline_step(
                ["src/utils/fetch_clinicaltrials.py", "--nct-ids", trial_id, "--output", ct_file],
                f"ClinicalTrials.gov for NCT '{trial_id}'"
            )
        else:
            run_pipeline_step(
                ["src/utils/fetch_clinicaltrials.py", "--terms", asset_name, "--output", ct_file, "--limit", "5"],
                f"ClinicalTrials.gov for asset '{asset_name}'"
            )
        if os.path.exists(ct_file):
            run_pipeline_step(
                ["src/utils/summarize_clinicaltrials.py", "--input", ct_file, "--output", ct_file.replace(".json", "_sum.txt")],
        ...
        if os.path.exists(fda_file):
            run_pipeline_step(
                ["src/utils/summarize_openfda.py", "--input", fda_file, "--output", fda_file.replace(".json", "_sum.txt")],
        ...
        run_pipeline_step(
            ["src/utils/fetch_pubchem.py", "--compound", asset_name, "--output", pubchem_file],
            f"PubChem compound lookup for '{asset_name}'"
        )
        if os.path.exists(pubchem_file):
            run_pipeline_step(
                ["src/utils/summarize_pubchem.py", "--input", pubchem_file, "--output", pubchem_file.replace(".json", "_sum.txt")],
    ...
    # Create the report document draft
    report_file = target_dir / "final_output" / f"deep_dive_{folder_safe_name}_{datetime.date.today().strftime('%Y%m%d')}.md"
    ...
    if ct_sum_path.exists():
        ct_sum = ct_sum_path.read_text(encoding="utf-8")
