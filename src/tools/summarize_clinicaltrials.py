...
def main():
    parser = argparse.ArgumentParser(
        description="Summarize ClinicalTrials.gov JSON data"
    )
    parser.add_argument(
        "--input", required=True, help="Path to the fetched trials JSON database"
    )
    parser.add_argument(
        "--output", required=True, help="Path to write the text summary report"
    )
    ...
    try:
        with open(args.output, "w", encoding="utf-8") as out:
            out.write("=" * 80 + "\n")
            out.write("CLINICAL TRIALS SUMMARY REPORT\n")
