...
def main():
    ...
    try:
        with open(args.output, "w", encoding="utf-8") as out:
            out.write("=" * 80 + "\n")
            out.write("OPENFDA DRUG SAFETY & LABEL SUMMARY REPORT\n")
            ...
            out.write(
                indications[:1500] + ("..." if len(indications) > 1500 else "") + "\n\n"
            )

            out.write("--- BOXED WARNINGS (CRITICAL TOXICITIES) ---\n")
            out.write(
                boxed_warning[:1500]
                + ("..." if len(boxed_warning) > 1500 else "")
                + "\n\n"
            )
            ...
            out.write(
                ...
                + ("..." if len(adverse_reactions) > 1500 else "")
                + "\n\n"
            )
            ...
            if event_summaries:
                ...
                out.write(
                    "No adverse event cases found in recent openFDA registries.\n"
                )
