...
def fetch_compound_data(name):
    ...
    if (
        not cid_data
        or "IdentifierList" not in cid_data
        or "CID" not in cid_data["IdentifierList"]
    ):
...
def main():
    parser = argparse.ArgumentParser(
        description="Query PubChem PUG REST API for BioAssay & Compound Data"
    )
    parser.add_argument(
        "--compound",
        required=True,
        help="Compound name (e.g., 'Aspirin' or 'Osemitamab')",
    )
    parser.add_argument(
        "--output", default="tmp/pubchem_out.json", help="Path to save raw JSON"
    )
