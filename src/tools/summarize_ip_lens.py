...
def main():
    parser = argparse.ArgumentParser(
        description="Summarize Global IP & Patent JSON data"
    )
    ...
    try:
        with open(args.output, "w", encoding="utf-8") as out:
            out.write("=" * 80 + "\n")
            out.write("GLOBAL PATENT & INTELLECTUAL PROPERTY (IP) REPORT\n")
            ...
            for idx, res in enumerate(results, 1):
                ...
                doi = res.get("doi", "N/A")
                abstract = res.get(
                    "abstractText", "No patent abstract details available."
                )
