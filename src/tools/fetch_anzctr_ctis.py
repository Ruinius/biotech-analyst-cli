...
def fetch_anzctr_ctis_trials(term, limit=50):
    ...
    print(
        f"Querying Europe PMC for trials matching: '{term}' in EU/Australian registries (limit: {limit})..."
    )
    ...
    while len(results) < limit:
        ...
        try:
            ...
            print(
                f"Error querying Europe PMC at cursor {cursor_mark}: {e}",
                file=sys.stderr,
            )
            break
...
def main():
    parser = argparse.ArgumentParser(
        description="Query Europe PMC for EU CTIS & Australian ANZCTR clinical trials"
    )
    parser.add_argument(
        "--term", required=True, help="Search term (e.g., drug name or target)"
    )
    parser.add_argument(
        "--output", default="tmp/anzctr_ctis_out.json", help="Path to save raw JSON"
    )
    parser.add_argument(
    ...
    if not results:
        print(
            f"Error: No clinical trials found for '{args.term}' inside EU CTIS / Australian ANZCTR registries.",
            file=sys.stderr,
        )
        sys.exit(1)
